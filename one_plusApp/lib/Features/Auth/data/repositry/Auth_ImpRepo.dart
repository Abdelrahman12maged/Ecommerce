...
class ImpAuthRepo extends baseAuthRepo {
  final baseAuthRemotData baseAuthRemoteDataSource;
  ...
  Future<Either<failure, UserEntity>> signUp(UserParams params) async {
    try {
      final user = await baseAuthRemoteDataSource.signUp(params);
      return Right(user);
    } catch (e) {
  ...
  Future<Either<failure, SignInModel>> signIn(UserParams params) async {
    try {
    final response=  await baseAuthRemoteDataSource.signIn(params);
